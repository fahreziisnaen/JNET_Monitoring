...
exports.getHotspotSummary = async (req, res) => {
    try {
        ...
        const [users, active] = await Promise.all([
            runCommandForWorkspace(workspaceId, '/ip/hotspot/user/print', [], deviceId).catch(() => []),
            runCommandForWorkspace(workspaceId, '/ip/hotspot/active/print', [], deviceId).catch(() => [])
        ]);
        
        // Pastikan selalu return nilai default meskipun error atau kosong
        res.json({
            totalUsers: Array.isArray(users) ? users.length : 0,
            activeUsers: Array.isArray(active) ? active.length : 0
        });
    } catch (error) {
        console.error('[Hotspot Summary] Error:', error.message);
...
exports.getHotspotUsers = async (req, res) => {
    try {
        const workspaceId = req.user.workspace_id;
        const deviceId = req.query.deviceId ? parseInt(req.query.deviceId) : null;
        const users = await runCommandForWorkspace(workspaceId, '/ip/hotspot/user/print', [], deviceId).catch(() => []);
        // Pastikan selalu return array, meskipun kosong
        res.json(Array.isArray(users) ? users : []);
    } catch (error) {
        console.error('[Hotspot Users] Error:', error.message);
...
exports.getHotspotProfiles = async (req, res) => {
    try {
        const profiles = await runCommandForWorkspace(req.user.workspace_id, '/ip/hotspot/user/profile/print').catch(() => []);
        // Pastikan selalu return array, meskipun kosong
        const profileNames = Array.isArray(profiles) ? profiles.map(p => p.name).filter(Boolean) : [];
        res.json(profileNames);
    } catch (error) {
        console.error('[Hotspot Profiles] Error:', error.message);
        // Return empty array jika ada error
        res.json([]);
    }
};
