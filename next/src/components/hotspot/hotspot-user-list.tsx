...
const HotspotUserList = ({ refreshTrigger, onActionComplete }: HotspotUserListProps) => {
  const { hotspotActive, selectedDeviceId } = useMikrotik() || { hotspotActive: [], selectedDeviceId: null };
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`${apiUrl}/api/hotspot/users?deviceId=${selectedDeviceId}`);
      if (!res.ok) {
                ...
                {allUsers.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center p-10 text-muted-foreground">
                      Tidak ada user hotspot yang terdeteksi.
                    </td>
                  </tr>
                ) : (
                  allUsers.map((user, i) => {
