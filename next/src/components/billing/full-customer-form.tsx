        ...
        <div className="px-6 py-5 overflow-y-auto space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
            <div className="sm:col-span-2">
              <label className={labelCls}>PPPoE Secret <span className="text-destructive">*</span> <span className="font-normal text-muted-foreground/70">— dari MikroTik (hanya yang belum terpakai)</span></label>
            ...
            <div>
              <label className={labelCls}>No. WhatsApp <span className="text-destructive">*</span></label>
          ...
          <div className="border-t pt-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-4">
              <div>
                <label className={labelCls}>Paket <span className="text-destructive">*</span></label>
                ...
                {packages.length === 0 && <p className="text-xs text-amber-600 mt-1.5">Belum ada paket. Buat paket dulu di tab Paket.</p>}
              </div>
              <div>
          ...
          <div className="border-t pt-4 space-y-4">
            <div>
              <label className={`${labelCls} flex items-center gap-1`}><MapPin size={14} /> Lokasi rumah <span className="text-destructive">*</span> <span className="font-normal text-muted-foreground/70">— klik / geser pin di peta</span></label>
              ...
              />
              <p className="text-[11px] text-muted-foreground mt-1.5">Tempel koordinat dari Google Maps (format: lintang, bujur).</p>
            </div>
            <div>
              <label className={labelCls}>Foto depan rumah <span className="font-normal text-muted-foreground/70">(opsional)</span></label>
              <div
                onClick={() => fileRef.current?.click()}
                onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
                onDragLeave={() => setDragOver(false)}
                onDrop={(e) => { e.preventDefault(); setDragOver(false); pickFile(e.dataTransfer.files?.[0] || null); }}
                className={`flex flex-col items-center justify-center gap-2 rounded-md border-2 border-dashed px-4 py-6 text-center cursor-pointer transition-colors ${dragOver ? 'border-primary bg-primary/5' : 'border-input hover:bg-accent/40'}`}
              >
                {photoUrl ? (
                  <>
