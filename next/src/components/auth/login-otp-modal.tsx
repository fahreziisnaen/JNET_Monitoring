    ...
    try {
      ...
      if (data.token && typeof window !== 'undefined') {
        try {
          ...
          console.log('[Login OTP] Token disimpan ke localStorage');
        } catch (e) {
          // localStorage mungkin tidak tersedia atau di-block (misalnya di UC Browser)
          console.warn('[Login OTP] Gagal menyimpan token ke localStorage, menggunakan cookie saja:', e);
        }
      }
      
      // Set user data dari response
      const userData = data.user || { id: userId, displayName: data.displayName, profile_picture_url: data.profile_picture_url };
      login(userData);
      
      // Tunggu sebentar untuk memastikan cookie ter-set
      await new Promise(resolve => setTimeout(resolve, 100));
      
      // Setelah login berhasil, cookie sudah di-set oleh backend
      // Trigger checkLoggedIn untuk verify cookie dan update state dengan data lengkap
      try {
        await checkLoggedIn();
        console.log('[Login OTP] CheckLoggedIn berhasil');
      } catch (checkError) {
        console.warn('[Login OTP] CheckLoggedIn error:', checkError);
        // Jika checkLoggedIn gagal tapi token ada, tetap lanjutkan
        if (data.token) {
          console.log('[Login OTP] Menggunakan token dari localStorage untuk melanjutkan');
        }
      }
