...
const AddClientModal = ({ isOpen, onClose, onSuccess, assets = [] }: AddClientModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setLongitude('');
      setOdpAssetId('');
      apiFetch(`${apiUrl}/api/clients/unlinked-pppoe-secrets`)
        .then(res => {
          if (!res.ok) throw new Error('Gagal memuat daftar PPPoE secrets.');
          return res.json();
        })
        .then(data => {
          setSecrets(data);
          if (data.length > 0) {
            const firstSecret = data[0];
        ...
        .catch(() => setError("Gagal memuat daftar PPPoE secrets."))
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    // Auto-select ODP jika PPPoE secret sudah terhubung ke ODP
                    const selectedSecretData = secrets.find(s => s.name === secretName);
